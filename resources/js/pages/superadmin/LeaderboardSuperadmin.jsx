...
export default function LeaderboardSuperadmin() {
  ...
  useEffect(() => {
    prevLeaderboardRef.current = leaderboardData;
  }, [leaderboardData]);
  ...
  const processCloseEvent = async () => {
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(result.message || "Gagal menutup sesi.");
      }
    } catch (error) {}
  };
      ...
      <div className="w-full max-w-md lg:max-w-6xl xl:max-w-7xl mx-auto min-h-screen flex flex-col relative">
        ...
        <div className="pt-12 px-6 flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-2">
            {sessionStatus !== 'ended' && (
              <button onClick={() => setModalState('end_event')} className="bg-[#E53E3E] text-white text-[12px] font-bold px-4 py-2 rounded-lg shadow-[0_3px_0_0_#B91C1C] active:translate-y-[3px] active:shadow-none hover:bg-[#DC2626] transition-all">
                Akhiri Sesi
              </button>
            )}

            {sessionStatus === 'ended' ? (
              <button onClick={() => navigate(`/superadmin/session/redeem/${id}`)} className="bg-[#2E9AD7] text-white text-[12px] font-bold px-4 py-2 rounded-lg shadow-[0_3px_0_0_#1C6B99] active:translate-y-[3px] active:shadow-none hover:bg-[#268bc4] transition-all">
                Redeem Hadiah
              </button>
            ) : (
            ...
            return (
              <div key={team.id} className="flex items-center bg-white rounded-[16px] py-2 px-4 lg:py-3 lg:px-8 border border-[#CBD5E1] shadow-sm hover:border-[#2E9AD7] hover:shadow-md transition-all group">
                ...
                <div className="flex-1 flex flex-col justify-center min-w-0 pr-4 lg:pr-8 border-l border-[#F1F5F9] pl-3 lg:pl-6 ml-2 lg:ml-0">
                  <p className="text-[#02101B] text-[14px] lg:text-[18px] font-bold truncate w-full">{team.name}</p>
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md rounded-full px-12 py-4 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1)] flex items-center gap-12 z-50 border border-[#F1F5F9]">
          <Link to={`/superadmin/session/live/${id}`} className="opacity-30 hover:opacity-100 hover:scale-110 transition-all">
            <img src={HomeIcon} alt="Home" className="w-7 h-7" />
          </Link>
          <button className="scale-110 opacity-100 drop-shadow-md cursor-default">
