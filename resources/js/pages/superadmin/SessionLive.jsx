...
const TeamCard = ({ team, selectedPosId, onSelesaiClick }) => {
  ...
  return (
    <div className="bg-white rounded-[20px] p-5 shadow-sm border border-white flex flex-col mb-4 relative z-10">
      ...
      {/* Tampilan jika status Pos adalah Active (Ongoing) atau Ready */}
      {state !== 'completed' && (
      ...
      {/* Tampilan jika status Pos sudah Selesai (Completed) */}
      {state === 'completed' && (
...
export default function SessionLiveSuperadmin() {
  ...
  // 1. Fetch Data Live dari Laravel
  const fetchLiveData = async () => {
    try {
      ...
      if (response.ok && result.success) {
        ...
        // Set Pos pertama sebagai default jika belum ada yang dipilih
        if (!selectedPos && result.session.posts.length > 0) {
          setSelectedPos(result.session.posts[0]);
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetchLiveData();
    // Bisa tambahkan setInterval di sini kalau mau auto-refresh
  }, [id]);
  ...
  // 2. Fungsi Simpan Nilai ke Laravel
  const handleSaveScore = async () => {
    try {
      ...
      if (response.ok && result.success) {
        closeModal();
        fetchLiveData(); // Refresh data supaya koin ter-update
      } else {
    ...
    <div className="min-h-screen bg-[#EBF2F8] font-sans flex justify-center pb-32">
      <div className="w-full max-w-md bg-[#EBF2F8] min-h-screen flex flex-col relative px-6 pt-12">
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-max min-w-[120px] bg-[#4B5563] border border-[#374151] rounded-xl shadow-lg py-1.5 flex flex-col z-50">
                {sessionData.posts.map((pos) => (
                  <button
                    key={pos.id}
                    onClick={() => { setSelectedPos(pos); setIsDropdownOpen(false); }}
                    className={`flex items-center gap-2 px-4 py-2 text-[13px] font-medium hover:bg-[#374151] transition-colors w-full text-left ${selectedPos.id === pos.id ? 'text-white' : 'text-gray-300'}`}
                  >
        ...
        <div className="text-center mb-8 relative z-10">
          <p className="text-[#92A0AD] text-[13px] font-medium mb-1">Durasi Maksimal</p>
          ...
          <div className="fixed inset-0 bg-[#EBF2F8]/80 backdrop-blur-[2px] z-50 flex items-center justify-center px-6">
            ...
            {modalState === 'scoring' && selectedTeam && (
              <div className="w-full max-w-[340px] bg-white rounded-[24px] p-6 shadow-2xl flex flex-col items-center">
                ...
                <input 
                  type="number" placeholder="0" value={scoreInput} onChange={(e) => setScoreInput(e.target.value)}
                  className="w-full border border-[#CBD5E1] rounded-[14px] py-3.5 px-4 text-center text-[18px] font-bold focus:outline-none focus:border-[#2E9AD7] mb-4"
                />
