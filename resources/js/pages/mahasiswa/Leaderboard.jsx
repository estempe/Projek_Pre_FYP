      ...
      <div className="w-full max-w-md min-h-screen flex flex-col relative">
        ...
        <div className="px-6 flex flex-col gap-4">
          {teams.map((team, index) => (
            <div key={team.id} className={`flex items-center bg-white rounded-[20px] p-4 ${team.name === namaTeam ? "border-2 border-[#02101B] shadow-sm" : "border border-dashed border-[#A0B0BC] shadow-sm"}`}>
              <div className="w-10 shrink-0">
                <span className={`text-[18px] font-bold ${team.name === namaTeam ? "text-[#02101B]" : "text-[#8C9BA5]"}`}>#{index + 1}</span>
              </div>
              <div className="flex-1 flex flex-col justify-center min-w-0 pr-2">
                {team.isFinished && <div className="w-fit bg-[#8900E8] text-white px-3 py-0.5 rounded-full text-[10px] font-bold tracking-wide mb-1">Selesai</div>}
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="font-medium text-[#E5A015] text-[16px]">{team.total_coins}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white rounded-full px-10 py-4 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1)] flex items-center gap-12 z-50">
          <button onClick={goToGameplay} className="opacity-40 hover:opacity-100 hover:scale-110 transition-all"><img src={HomeIcon} alt="Home" className="w-7 h-7" /></button>
          <button className="hover:scale-110 opacity-100 transition-transform" disabled><img src={TrophyIcon} alt="Reward" className="w-7 h-7" /></button>
        </div>
      </div>
    </div>
  );
